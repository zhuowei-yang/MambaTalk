...
class LocalScan(nn.Module):
    def __init__(self, args):
        ...
        self.motion_res_body = nn.Linear(args.hidden_size, latent_dim)
        self.motion_res_hands = nn.Linear(args.hidden_size, latent_dim)
    ...
    def forward(self, global_motions, global_features, face_latent_in, speaker_embedding_body, body_features, use_word):
        ...
        # codebook
        body_latent_raw = motion_body + body_latent
        hands_latent_raw = motion_hands + hands_latent
        face_latent = self.motion_down_face(decoded_face)
        body_latent = self.motion_down_body(body_latent_raw)
        hands_latent = self.motion_down_hands(hands_latent_raw)
        ...
        res_body = self.motion_res_body(body_latent_raw)
        res_hands = self.motion_res_hands(hands_latent_raw)
