...
class VQVAEConvZeroRes(nn.Module):
    """VQ-VAE with continuous residual bypass: z = z_q + z_res."""
    def __init__(self, args):
        super().__init__()
        self.encoder = VQEncoderV5(args)
        self.quantizer = Quantizer(args.vae_codebook_size, args.vae_length, args.vae_quantizer_lambda)
    ...
    def forward(self, inputs):
        ...
        embedding_loss, z_q, _, perplexity = self.quantizer(z_e)
        z_res = (z_e - z_q).detach()
        z_combined = z_q + self.res_scale * z_res
        rec_pose = self.decoder(z_combined)
        return {
            "poses_feat": z_q,
            "embedding_loss": embedding_loss,
            "perplexity": perplexity,
    ...
    def map2index(self, inputs):
        z_e = self.encoder(inputs)
        return self.quantizer.map2index(z_e)

    def map2latent(self, inputs):
        z_e = self.encoder(inputs)
        index = self.quantizer.map2index(z_e)
        return self.quantizer.get_codebook_entry(index)

    def map2residual(self, inputs):
        z_e = self.encoder(inputs)
        index = self.quantizer.map2index(z_e)
        z_q = self.quantizer.get_codebook_entry(index)
        return z_e - z_q

    def decode(self, index, z_res=None):
        z_q = self.quantizer.get_codebook_entry(index)
