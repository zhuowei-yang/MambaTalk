...
BODY_CKPT = "./outputs/audio2pose/custom/0305_053835_mhr_cont_body/rec.bin"
HAND_CKPT = "./outputs/audio2pose/custom/0305_054312_mhr_cont_hand/rec.bin"
...
def load_vae(ckpt_path, vae_test_dim, vae_length=256, vae_codebook_size=256, vae_layer=2):
    ...
    mod = __import__("models.motion_representation", fromlist=["VQVAEConvZero"])
    model = getattr(mod, "VQVAEConvZero")(args).cuda().eval()
...
def render_npz(npz_path, gpu):
    ...
    if result.returncode != 0:
        print(f"    RENDER STDERR (tail): {result.stderr[-800:]}")
    if result.stdout:
        print(f"    RENDER STDOUT (tail): {result.stdout[-500:]}")
...
def main():
    ...
    if not args.skip_inference:
        ...
        print("=" * 60)
        print(f"Body VAE: {args.body_ckpt}")
        print(f"Hand VAE: {args.hand_ckpt}")

        body_vae = load_vae(args.body_ckpt, vae_test_dim=BODY_CONT_DIM)
