...
SAMPLES="VWeRsVnYMMQ_0010_speaker1 ISgz9wLZxJI_0032_speaker1 -1uuDfD0Uec_0031_speaker1"
...
for name in $SAMPLES; do
    ...
    # Truncate GT to match pred frame count to avoid IndexError
    pred_fixed="${OUT_DIR}/pred_${name}.npz"
    gt_copy="${OUT_DIR}/gt_${name}.npz"
    python3 -c "
import numpy as np
pred = dict(np.load('${pred_npz}', allow_pickle=True))
gt = dict(np.load('${gt_npz}', allow_pickle=True))
N_pred = len(pred['body_pose_params'])
N_gt = len(gt['body_pose_params'])
N = min(N_pred, N_gt)

for d, path in [(pred, '${pred_fixed}'), (gt, '${gt_copy}')]:
    out = {}
    for k, v in d.items():
        if hasattr(v, 'shape') and len(v.shape) > 0 and v.shape[0] in (N_pred, N_gt):
            out[k] = v[:N]
        elif hasattr(v, 'shape') and len(v.shape) > 0 and v.shape[0] == 1:
            out[k] = np.tile(v, (N,) + (1,)*(len(v.shape)-1))
        else:
            out[k] = v
    np.savez(path, **out)
print(f'Frames: pred={N_pred}, gt={N_gt}, used={N}')
"
