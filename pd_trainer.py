"""
Standalone pre-training script for the Periodicity Disentanglement (PD) module.

Reads npz files directly, computes joint velocity from pred_joint_coords,
and trains the PD autoencoder independently from the main MambaTalk pipeline.

Usage:
    CUDA_VISIBLE_DEVICES=0 python pd_trainer.py --config configs/pd_config.yaml
"""
...
from models.periodicity_module import PeriodicityDisentanglement
...
def train(cfg: dict):
    ...
    model = PeriodicityDisentanglement(
    ...
    for epoch in range(1, cfg["epochs"] + 1):
        ...
        if epoch_losses["total"] < best_loss:
            ...
            torch.save(model.state_dict(), os.path.join(out_dir, "pd_best.bin"))

        if epoch % cfg.get("save_period", 20) == 0:
            torch.save(model.state_dict(), os.path.join(out_dir, f"pd_epoch_{epoch}.bin"))

    torch.save(model.state_dict(), os.path.join(out_dir, "pd_final.bin"))
    logger.info(f"Training complete. Best loss: {best_loss:.6f}. Saved to {out_dir}")


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PD Module Pre-training")
    parser.add_argument("--config", type=str, default="configs/pd_config.yaml")
