...
class CustomTrainer(train.BaseTrainer):
    def __init__(self, args):
        ...
        self.args.vae_codebook_size = VAE_CODEBOOK
        ...
        self.proj_body = nn.Linear(VAE_CODEBOOK, VAE_LATENT).to(self.rank)
        self.proj_hands = nn.Linear(VAE_CODEBOOK, VAE_LATENT).to(self.rank)
        self.proj_face = nn.Linear(VAE_CODEBOOK, VAE_LATENT).to(self.rank)
    ...
    def _g_training(self, loaded_data, use_adv, mode="train", epoch=0):
        ...
        rec_index_face_val = self.log_softmax(net_out_val["cls_face"]).reshape(-1, VAE_CODEBOOK)
        rec_index_body_val = self.log_softmax(net_out_val["cls_body"]).reshape(-1, VAE_CODEBOOK)
        rec_index_hands_val = self.log_softmax(net_out_val["cls_hands"]).reshape(-1, VAE_CODEBOOK)
        ...
        loss_cls = self.args.cf * self.cls_loss(rec_index_face_val, tar_index_face) \
            + self.args.cu * self.cls_loss_smooth(net_out_val["cls_body"].reshape(-1, VAE_CODEBOOK), tar_index_body) \
        ...
        if mode == 'train':
            ...
            index_loss_self = self.cls_loss_smooth(net_out_self["cls_body"].reshape(-1, VAE_CODEBOOK), tar_index_body) \
                + self.cls_loss(self.log_softmax(net_out_self["cls_hands"]).reshape(-1, VAE_CODEBOOK), tar_index_hands)
            ...
            index_loss_word = self.cls_loss_smooth(net_out_word["cls_body"].reshape(-1, VAE_CODEBOOK), tar_index_body) \
                + self.cls_loss(self.log_softmax(net_out_word["cls_hands"]).reshape(-1, VAE_CODEBOOK), tar_index_hands)
            self.tracker.update_meter("cls_word", "train", index_loss_word.item())
            g_loss_final += index_loss_word

        if mode != 'train':
            if self.args.cu != 0:
                _, rec_idx_body = torch.max(rec_index_body_val.reshape(-1, self.args.pose_length, VAE_CODEBOOK), dim=2)
            ...
            if self.args.ch != 0:
                _, rec_idx_hands = torch.max(rec_index_hands_val.reshape(-1, self.args.pose_length, VAE_CODEBOOK), dim=2)
    ...
    def _g_test(self, loaded_data):
        ...
        for i in range(roundt):
            ...
            if self.args.cu != 0:
                rec_idx_body = self.log_softmax(net_out_val["cls_body"]).reshape(-1, VAE_CODEBOOK)
                _, rec_idx_body = torch.max(rec_idx_body.reshape(-1, self.args.pose_length, VAE_CODEBOOK), dim=2)
            else:
                _, rec_idx_body, _, _ = self.vq_model_body.quantizer(net_out_val["rec_body"])
            if self.args.ch != 0:
                rec_idx_hands = self.log_softmax(net_out_val["cls_hands"]).reshape(-1, VAE_CODEBOOK)
                _, rec_idx_hands = torch.max(rec_idx_hands.reshape(-1, self.args.pose_length, VAE_CODEBOOK), dim=2)
            else:
                _, rec_idx_hands, _, _ = self.vq_model_hands.quantizer(net_out_val["rec_hands"])
