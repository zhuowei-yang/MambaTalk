...
class CustomTrainer(train.BaseTrainer):
    def __init__(self, args):
        ...
        self.tracker = other_tools.EpochTracker(
            ["fid", "l1div", "bc", "rec", "trans", "vel", "transv", 'dis', 'gen', 'acc',
             ...
             "cls_full", "cls_self", "cls_word", "latent_word", "latent_self", "accel", "res"],
            [False, True, True] + [False] * 21
        )
        ...
        self.vq_model_body = getattr(vq_model_module, "VQVAEConvZeroRes")(self.args).to(self.rank)
        ...
        self.vq_model_hands = getattr(vq_model_module, "VQVAEConvZeroRes")(self.args).to(self.rank)
    ...
    def _load_data(self, dict_data):
        ...
        tar_pose_global = tar_pose_raw[:, :, GLOBAL_FULL_SLICE[0]:GLOBAL_FULL_SLICE[1]]
        tar_pose_face = torch.zeros(bs, n, 75, device=self.rank)

        tar_trans = dict_data["trans"].to(self.rank)
        tar_exps = torch.zeros(bs, n, 72, device=self.rank)
        tar_contact = tar_pose_raw[:, :, CONTACT_SLICE[0]:CONTACT_SLICE[1]]
        ...
        tar_res_body = self.vq_model_body.map2residual(tar_pose_body)
        tar_res_hands = self.vq_model_hands.map2residual(tar_pose_hands)
        ...
        return {
            ...
            "tar_res_body": tar_res_body,
            "tar_res_hands": tar_res_hands,
        }
    ...
    def _g_training(self, loaded_data, use_adv, mode="train", epoch=0):
        ...
        loss_vel_global = self.vel_loss(rec_global[:, 1:] - rec_global[:, :-1], tar_global[:, 1:] - tar_global[:, :-1])
        ...
        res_pred_w = getattr(self.args, 'res_pred_weight', 1.0)
        loss_res = res_pred_w * (
            self.reclatent_loss(net_out_val["res_body"], loaded_data["tar_res_body"])
            + self.reclatent_loss(net_out_val["res_hands"], loaded_data["tar_res_hands"])
        )
        self.tracker.update_meter("res", "train", loss_res.item())
        g_loss_final += loss_res
        ...
        if mode == 'train':
            ...
            loss_latent_self = self.args.lf * self.reclatent_loss(self.proj_face(net_out_self["rec_face"]), loaded_data["latent_face"]) \
                + self.args.ll * self.reclatent_loss(net_out_self["rec_global"], loaded_data["latent_global"]) \
            ...
            loss_latent_word = self.args.lf * self.reclatent_loss(self.proj_face(net_out_word["rec_face"]), loaded_data["latent_face"]) \
                + self.args.ll * self.reclatent_loss(net_out_word["rec_global"], loaded_data["latent_global"]) \
        ...
        if mode != 'train':
            if self.args.cu != 0:
                ...
                rec_body = self.vq_model_body.decode(rec_idx_body, z_res=net_out_val["res_body"])
            else:
            ...
            if self.args.ch != 0:
                ...
                rec_hands = self.vq_model_hands.decode(rec_idx_hands, z_res=net_out_val["res_hands"])
            else:
            ...
            rec_global = net_out_val["rec_global"]
            rec_pose = torch.cat([rec_body, rec_hands, rec_global], dim=-1)
    ...
    def _g_test(self, loaded_data):
        ...
        for i in range(roundt):
            ...
            rec_res_body = net_out_val["res_body"]
            rec_res_hands = net_out_val["res_hands"]
            ...
            if i == 0:
                ...
                rec_res_all_body.append(rec_res_body)
                rec_res_all_hands.append(rec_res_hands)
            else:
                ...
                rec_res_all_body.append(rec_res_body[:, self.args.pre_frames:])
                rec_res_all_hands.append(rec_res_hands[:, self.args.pre_frames:])

            rec_body_last = self.vq_model_body.decode(rec_idx_body, z_res=rec_res_body) if self.args.cu != 0 else self.vq_model_body.decoder(rec_idx_body)
            rec_hands_last = self.vq_model_hands.decode(rec_idx_hands, z_res=rec_res_hands) if self.args.ch != 0 else self.vq_model_hands.decoder(rec_idx_hands)
            latent_last = torch.cat([rec_body_last, rec_hands_last, rec_global_latent], dim=-1)
        ...
        rec_res_body_all = torch.cat(rec_res_all_body, dim=1)
        rec_res_hands_all = torch.cat(rec_res_all_hands, dim=1)

        rec_body = self.vq_model_body.decode(rec_index_body, z_res=rec_res_body_all) if self.args.cu != 0 else self.vq_model_body.decoder(rec_index_body)
        rec_hands = self.vq_model_hands.decode(rec_index_hands, z_res=rec_res_hands_all) if self.args.ch != 0 else self.vq_model_hands.decoder(rec_index_hands)
        rec_global = rec_latent_global
