...
Extended with FFT Periodicity Disentanglement (PD) module integration.
"""
import train
import os
import time
...
class CustomTrainer(train.BaseTrainer):
    def __init__(self, args):
        ...
        # v8: FreqPeriodicBlock as frozen pseudo-label extractor (velocity space)
        ...
        if self.use_periodic:
            ...
            if pd_ckpt and os.path.exists(pd_ckpt):
                ...
                logger.info(f"Loaded FreqPD checkpoint for pseudo-labels: {pd_ckpt}")
            else:
                raise RuntimeError(f"FreqPD checkpoint required but not found at '{pd_ckpt}'. "
                                   "Disable PD by setting pd_n_channels=0 or provide a valid checkpoint.")
            self.pd_module.eval()
    ...
    def _load_data(self, dict_data):
        ...
        if self.use_periodic:
            if "joint_velocity" in dict_data:
                jvel = dict_data["joint_velocity"].to(self.rank)
                if jvel.abs().sum() > 0:
                    ...
                    logger.warning("joint_velocity is all zeros, skipping PD pseudo-labels")
            else:
                logger.warning("joint_velocity not found in batch data")
