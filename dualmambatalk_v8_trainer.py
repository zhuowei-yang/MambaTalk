"""
DualMambaTalkV8 Trainer.
Extends the v8 MHR trainer for dual-person gesture generation with:
  - Paired speaker1/speaker2 data loading
  - Dialogue state conditioning via SoulX-Duplug
  - InteractionFusion between GlobalScan and LocalScan
  - All v8 losses preserved (latent MSE, vel, acc, codebook CE/NLL, periodic)
"""
import train
import os
import time
...
from dataloaders.beat_mhr import JOINT_VEL_DIM
...
class CustomTrainer(train.BaseTrainer):
    def __init__(self, args):
        ...
        # v8: FreqPeriodicBlock as frozen pseudo-label extractor
        self.use_periodic = getattr(args, 'pd_n_channels', 0) > 0
        self.pd_module = None
        if self.use_periodic:
            pd_ckpt = getattr(args, 'pd_checkpoint', '')
            self.pd_module = FreqPeriodicBlock(
                input_dim=JOINT_VEL_DIM,
                n_channels=args.pd_n_channels,
            ).to(self.rank)
            if pd_ckpt and os.path.exists(pd_ckpt):
                state = torch.load(pd_ckpt, map_location=f'cuda:{self.rank}')
                self.pd_module.load_state_dict(state)
                logger.info(f"Loaded FreqPD checkpoint: {pd_ckpt}")
            else:
                raise RuntimeError(f"FreqPD checkpoint not found: '{pd_ckpt}'")
            self.pd_module.eval()
            for p in self.pd_module.parameters():
                p.requires_grad = False
            self.periodic_mse = nn.MSELoss().to(self.rank)
            self.motion_l1 = nn.L1Loss().to(self.rank)
            self.lambda_p = getattr(args, 'lambda_p', 0.1)
            self.lambda_motion = getattr(args, 'lambda_motion', 0.1)

    def _load_data(self, dict_data):
        """Load paired dual-person data. Speaker1 = condition, Speaker2 = target."""
        # --- condition (speaker1) ---
        cond_pose = dict_data["pose1"].to(self.rank)
        cond_audio = dict_data["audio1"].to(self.rank)
        state1 = dict_data["state1"].to(self.rank)
        ...
        state2 = dict_data["state2"].to(self.rank)
        ...
        if self.use_periodic:
            if "jvel2" in dict_data:
                jvel = dict_data["jvel2"].to(self.rank)
                if jvel.abs().sum() > 0:
                    result["joint_velocity"] = jvel
                    with torch.no_grad():
                        pd_params = self.pd_module.extract_params(jvel)
                    result["pd_params_gt"] = pd_params

        return result

    def _model_forward(self, loaded_data, mask, use_word=True):
        """Run DualMambaTalkV8 forward with all dual-person inputs."""
        return self.model(
            ...
            state_ids=loaded_data['state_ids'],
            use_attentions=True,
            use_word=use_word,
        )
    ...
    def _g_training(self, loaded_data, use_adv, mode="train", epoch=0):
        ...
        net_out_val = self._model_forward(loaded_data, mask_val)
        g_loss_final = 0
        ...
        loss_latent = (self.args.lf * loss_latent_face + self.args.ll * loss_latent_global
                       + self.args.lh * loss_latent_hands + self.args.lu * loss_latent_body)
        ...
        loss_vel = (self.args.vel_global_weight * loss_vel_global
                    + self.args.vel_body_weight * loss_vel_body
                    + self.args.vel_hands_weight * loss_vel_hands)
        ...
        loss_acc = (self.args.acc_global_weight * loss_acc_global
                    + self.args.acc_body_weight * loss_acc_body
                    + self.args.acc_hands_weight * loss_acc_hands)
        ...
        # --- self-supervised: mask annealing with dual conditions ---
        if mode == 'train':
            mask_ratio = (epoch / self.args.epochs) * 0.95 + 0.05
            mask = torch.rand(bs, n, self.args.pose_dims) < mask_ratio
            mask = mask.float().cuda()

            net_out_self = self._model_forward(loaded_data, mask, use_word=False)
            loss_latent_self = (
                self.args.lf * self.reclatent_loss(self.proj_face(net_out_self["rec_face"]), loaded_data["latent_face"])
            ...
            index_loss_self = (
                self.cls_loss_smooth(net_out_self["cls_body"].reshape(-1, BODY_CODEBOOK), tar_index_body)
            ...
            net_out_word = self._model_forward(loaded_data, mask, use_word=True)
            loss_latent_word = (
                self.args.lf * self.reclatent_loss(self.proj_face(net_out_word["rec_face"]), loaded_data["latent_face"])
            ...
            index_loss_word = (
                self.cls_loss_smooth(net_out_word["cls_body"].reshape(-1, BODY_CODEBOOK), tar_index_body)
        ...
        # --- v8 periodic loss ---
        if self.use_periodic and "periodic_params" in net_out_val and "pd_params_gt" in loaded_data:
            pred_p = net_out_val["periodic_params"]
            gt_p = loaded_data["pd_params_gt"]
            loss_periodic = sum(
                self.periodic_mse(pred_p[k], gt_p[k]) for k in ("A", "F", "B", "S"))
            g_loss_final += self.lambda_p * loss_periodic
            self.tracker.update_meter("periodic", "train", loss_periodic.item())

            if "periodic_body" in net_out_val:
                if self.args.cu != 0:
                    _, idx_b_tmp = torch.max(rec_index_body_val.reshape(-1, self.args.pose_length, BODY_CODEBOOK), dim=2)
                    dec_body = self.vq_model_body.decode(idx_b_tmp)
                else:
                    _, idx_b_tmp, _, _ = self.vq_model_body.quantizer(net_out_val["rec_body"])
                    dec_body = self.vq_model_body.decoder(idx_b_tmp)
                if self.args.ch != 0:
                    _, idx_h_tmp = torch.max(rec_index_hands_val.reshape(-1, self.args.pose_length, HAND_CODEBOOK), dim=2)
                    dec_hands = self.vq_model_hands.decode(idx_h_tmp)
                else:
                    _, idx_h_tmp, _, _ = self.vq_model_hands.quantizer(net_out_val["rec_hands"])
                    dec_hands = self.vq_model_hands.decoder(idx_h_tmp)
                enh_body = dec_body + net_out_val["periodic_body"]
                enh_hands = dec_hands + net_out_val["periodic_hands"]
                loss_motion = (self.motion_l1(enh_body, loaded_data["tar_pose_body"])
                               + self.motion_l1(enh_hands, loaded_data["tar_pose_hands"]))
                g_loss_final += self.lambda_motion * loss_motion
                self.tracker.update_meter("motion_enh", "train", loss_motion.item())

        # --- val/test: reconstruct full pose ---
        if mode != 'train':
            if self.args.cu != 0:
            ...
            if self.use_periodic and "periodic_body" in net_out_val:
                rec_body = rec_body + net_out_val["periodic_body"]
                rec_hands = rec_hands + net_out_val["periodic_hands"]
            rec_rot6d_val = net_out_val["rec_global"]
            rec_pose = torch.cat([rec_body, rec_hands, rec_rot6d_val,
                                  loaded_data["tar_pose_cam_t"],
                                  loaded_data["tar_pose_contact"]], dim=-1)
    ...
    def _g_test(self, loaded_data):
        """Autoregressive test generation with dual-person conditioning."""
        bs, n, _ = loaded_data["tar_pose"].shape
        ...
        cond_pose_full = loaded_data["cond_pose"]
        cond_audio_full = loaded_data["cond_audio"]
        state_ids_full = loaded_data["state_ids"]
        ...
        for i in range(roundt):
            ...
            cond_pose_tmp = cond_pose_full[:, s_e:e_e, :]
            cond_audio_tmp = cond_audio_full[:, i * (16000 // 30 * round_l):(i + 1) * (16000 // 30 * round_l) + 16000 // 30 * self.args.pre_frames]
            state_tmp = state_ids_full[:, s_e:e_e] if state_ids_full.shape[1] > s_e else state_ids_full
            ...
            net_out_val = self.model(
                ...
                cond_audio=cond_audio_tmp, cond_motion=cond_pose_tmp,
                state_ids=state_tmp, use_attentions=True)
            ...
            if i == 0:
                ...
                if self.use_periodic and "periodic_body" in net_out_val:
                    periodic_body_all.append(net_out_val["periodic_body"])
                    periodic_hands_all.append(net_out_val["periodic_hands"])
                if "sigma_weights" in net_out_val:
                    sigma_all.append(net_out_val["sigma_weights"])
            else:
                ...
                if self.use_periodic and "periodic_body" in net_out_val:
                    periodic_body_all.append(net_out_val["periodic_body"][:, self.args.pre_frames:])
                    periodic_hands_all.append(net_out_val["periodic_hands"][:, self.args.pre_frames:])
                if "sigma_weights" in net_out_val:
                    sigma_all.append(net_out_val["sigma_weights"][:, self.args.pre_frames:])
        ...
        if self.use_periodic and len(periodic_body_all) > 0:
            pe_body = torch.cat(periodic_body_all, dim=1)
            pe_hands = torch.cat(periodic_hands_all, dim=1)
            min_t = min(rec_body.shape[1], pe_body.shape[1])
            rec_body = rec_body[:, :min_t, :] + pe_body[:, :min_t, :]
            rec_hands = rec_hands[:, :min_t, :] + pe_hands[:, :min_t, :]
            rec_rot6d_final = rec_rot6d_final[:, :min_t, :]
            rec_cam_t_final = rec_cam_t_final[:, :min_t, :]
            rec_contact_final = rec_contact_final[:, :min_t, :]
        ...
        result = {
            'rec_pose': rec_pose, 'rec_trans': tar_trans,
            'tar_pose': tar_pose, 'tar_exps': tar_exps,
            'tar_beta': tar_beta, 'tar_trans': tar_trans,
        }
        if sigma_all:
            sigma_cat = torch.cat(sigma_all, dim=1)
            result['sigma_weights'] = sigma_cat[:, :n, :]
        return result
    ...
    def test(self, epoch):
        ...
        with torch.no_grad():
            for its, batch_data in enumerate(self.test_loader):
                ...
                gt_npz = np.load(
                    self.args.data_path + self.args.pose_rep + "/"
                    + speaker2_id + ".npz", allow_pickle=True)
                ...
                body_euler, hand_pca, expr, global_rot, _ = self._cont_to_mhr_npz(rec_np)

                cam_t_seq = tar_trans.detach().cpu().numpy().reshape(bs * n_out, -1)
                cam_t_mean = cam_t_seq.mean(axis=0, keepdims=True).repeat(bs * n_out, axis=0)

                save_dict = dict(
                    body_pose_params=body_euler, hand_pose_params=hand_pca,
                    expr_params=expr, global_rot=global_rot, pred_cam_t=cam_t_mean,
                    **render_consts)
                if 'sigma_weights' in net_out:
                    sw = net_out['sigma_weights'].detach().cpu().numpy()
                    save_dict['sigma_weights'] = sw.reshape(-1, 3)[:n_out]
                np.savez(
                    results_save_path + "res_" + speaker2_id + '.npz',
                    **save_dict)
                total_length += n_out
